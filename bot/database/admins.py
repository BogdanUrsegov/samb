...
def _session_factory():
    # Imported lazily because session.py uses ensure_superadmin during startup.
...
async def is_superadmin(user_id: int) -> bool:
    async with _session_factory()() as session:
        result = await session.execute(select(Admin.is_superadmin).where(Admin.user_id == user_id))
...
async def add_admin(user_id: int, added_by: int, is_superadmin_flag: bool = False) -> bool:
    async with _session_factory()() as session:
        async with session.begin():
            ...
            if existing:
                return False
            session.add(Admin(user_id=user_id, added_by=added_by, is_superadmin=is_superadmin_flag))
